...
const updatePastLectures = async () => {
  try {
    ...
    const updatePromises = pastLectures.map(lecture => {
      ...
      sendEmail({
        to: recipients,
        subject: `Lecture have started for ${lecture.title}`,
        html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px;">Lecture have started for ${lecture.title}</h2>
            
            <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
                <p style="font-size: 16px;">Hello,</p>
                <p>Your lecture has started and also your presence is needed as well</p>
            ...
            <p style="text-align: center; color: #7f8c8d;">
                <a href="${process.env.CLIENT_URL}/online-class" style="color: #3498db; text-decoration: none;">View in MyTeacher</a>
            </p>
        </div>
        `
      });
